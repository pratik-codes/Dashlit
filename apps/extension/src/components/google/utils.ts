...
export const getCalendarEvents = async (token: any) => {
  ...
  try {
    ...
    fetch(`${apiUrl}/calendars/${encodeURIComponent(calendarId)}/events?timeMin=${currentDate}&maxResults=10&singleEvents=true&orderBy=startTime`, {
      headers: {
...
export const getEmails = async (token: any) => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.error('Error retrieving emails:', response.status, response.statusText);
    }
  } catch (error) {
  ...
  try {
    if (window.location.origin.includes('chrome-extension')) {
      chrome.identity.getAuthToken(
        { interactive: true, scopes: ['https://www.googleapis.com/auth/calendar.readonly', 'https://www.googleapis.com/auth/gmail.readonly'] },
        async (token) => {
